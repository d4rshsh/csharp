...
namespace Sep14Exercises
{
    internal class Program
    {
        ...
        {
            CustomerUser cust1 = new CustomerUser();
            cust1.Id = 7358952929;
            cust1.Password = "sesh2002";
            cust1.validate();
            CustomerUser cust2 = new CustomerUser();
            cust2.Id = 7358952929;
            cust2.Password = "sesh2002";
            cust2.validate();
            FileStream fs = new FileStream(@"empty.bin", FileMode.Create, FileAccess.Write);
            BinaryFormatter binformat = new BinaryFormatter(); // serializes and deserializes an object
            binformat.Serialize(fs, cust1);
            binformat.Serialize(fs, cust2);
            fs.Flush();
            fs.Close();
            fs.Dispose();
        ...
        mer:
            ...
            cust1.alreadyseen = dgg;
            FileStream fs4 = new FileStream(@"empty2.Xml", FileMode.Create, FileAccess.Write);
            XmlSerializer xm = new System.Xml.Serialization.XmlSerializer(typeof(CustomerUser));
            xm.Serialize(fs4, cust1);
            fs4.Flush();
            fs4.Close();
            fs4.Dispose();
